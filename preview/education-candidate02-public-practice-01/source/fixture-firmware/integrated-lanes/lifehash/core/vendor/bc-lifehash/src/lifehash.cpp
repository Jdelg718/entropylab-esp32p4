// LOCAL ADAPTATION revision-05: exception-safe grid ownership; see ADAPTATION.md.
#include <algorithm>
#include <cstring>
#include <memory>
#include <set>
...
Image make_from_digest(const std::vector<uint8_t>& digest, Version version, size_t module_size, bool has_alpha) {
    ...
    auto current_cell_grid = std::make_unique<CellGrid>(size);
    auto next_cell_grid = std::make_unique<CellGrid>(size);
    auto current_change_grid = std::make_unique<ChangeGrid>(size);
    auto next_change_grid = std::make_unique<ChangeGrid>(size);
