 ...
 def test_refreshed_matrix(self):
  with tempfile.TemporaryDirectory() as tmp:
   ...
   for i in range(len(json.loads(original)['entries'])):
    candidate_error='global-saver chain discontinuity' if i<2 else 'review-repair current'
    for field,value,error in [('before_sha256','0'*64,'review-repair chain'),('classification','invalid','classification/path'),('reason','','reason'),('candidate_sha256','0'*64,candidate_error)]:
