...
assert sha(a.archive.read_bytes()) == '6ff7ca8779850f1277e61a5d7dc875e24fd1c4c921601a4ea47b184e6d497cd9'
...
with tempfile.TemporaryDirectory(prefix='successor-closeout-') as tmp:
    ...
    node = subprocess.run(['node', '--test', 'tests/successor-smoke.test.mjs', 'flash/first-install/wire.test.mjs'], cwd=root, capture_output=True, text=True)
    (a.output/'node-smoke.log').write_text(node.stdout + node.stderr)
    assert node.returncode == 0
    result['node'] = 'PASS (40 tests)'
    ...
    cases = {
        ...
        'hold-reintroduced': mutated(adapter_name, adapter.replace(b'"writeHold": false', b'"writeHold": true')),
        'security-bypass': mutated(adapter_name, adapter.replace(b"if (s.chipId !== 18)", b"if (false)")),
    ...
    try:
        with sync_playwright() as p:
            ...
            for width, motion in [(320, 'no-preference'), (390, 'reduce'), (1280, 'no-preference')]:
                ...
                assert pins['profile']['writeHold'] is False
                ...
                # Synthetic chooser only: no real serial access, never open this port.
                page.evaluate("Object.defineProperty(navigator,'serial',{configurable:true,value:{requestPort:async()=>({})}})")
                page.locator('#board').check()
                page.locator('#connect').click()
                page.wait_for_function("document.querySelector('#status').textContent.startsWith('Port selected, NOT opened.')")
                assert page.locator('#check').is_enabled()
                assert page.locator('#flash').is_disabled()
                page.locator('#consent').check()
                assert page.locator('#flash').is_enabled()
            ...
            page.goto(origin+'/flash/first-install/')
            result['browser_transport'] = page.evaluate("async () => {const m=await import('./speed-browser.mjs');return await m.run();}")
            assert len(result['browser_transport']) == 16
            page.close()
