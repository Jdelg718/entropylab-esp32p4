...
## Historical roadmap and combined-source checkpoint

Everything below is retained historical direction/evidence, not current priority or implementation status. Optional passphrase, LifeHash and additional input methods are now implemented in native source; that does not qualify every feature on hardware.
