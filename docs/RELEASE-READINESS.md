...
The one-board install/boot/result-retention milestone is accepted, not blocked by the earlier automated port chooser attempt. The current candidate is **unpublished**, with unchanged accepted firmware and archive. No extra flash is needed to repeat that accepted milestone. Runtime `dc5c65420aa96013ae3847ab4771a7f520f1c3a8` is the historically qualified speed/progress implementation. This local successor integrates education copy, readiness documentation and browser progress animation; that integration does not extend historical qualification to the new source or animation.

### Integrated source versus accepted binaries

The shipped firmware images and accepted source archive remain immutable. In contrast, `fixture-firmware/app/main/education_content.inc` now contains successor lessons that have **not** been target-built or installed. The bundled accepted images do not contain those lessons. Native education host tests exercise successor source, not the shipped binaries; browser synthetic-port tests exercise installer behavior, not hardware acceptance.

The accepted-source checks intentionally reject this combined checkout: `scripts/retention-host.py --check` reports education source drift, `scripts/test-documentation-successors.py` fails its initial accepted-source precondition, and `scripts/review-public.py` rejects fixture-source equality after its notice checks. These are not the separate known legacy `gui.c` host failure. Do not alter immutable pins, broaden documentation exceptions to firmware source, or substitute historical content to make these checks pass. The readiness ledger checks historical evidence coherence only.

A fresh official release package is deferred pending a reviewed successor source/binary provenance contract that preserves this distinction and passes its own review gates. No accepted asset replacement or release approval follows from this integration.
...
The historical accepted native regression route is `python3 scripts/retention-host.py` with its [prerequisites](retention-host/README.md); it runs archived source and cannot certify the integrated education successor. For the live education source, use `EDUCATION_ONLY=1 bash scripts/test-gui-host.sh` with the documented native dependencies. The mandated legacy `scripts/test-host.sh` rejects historical GUI identity on this successor; preserve and report that failure, never repin it to claim PASS. Historical evidence and current execution results must be reported separately.
